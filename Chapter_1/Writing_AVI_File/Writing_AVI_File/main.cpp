...
int main()
{
	...
	*/
	...
	std::cout << "dddddddddddddddddddddddddddddddd\n" << capture.get(CAP_PROP_FRAME_WIDTH) <<"\n";
	std::cout << "dddddddddddddddddddddddddddddddd\n" << capture.get(CAP_PROP_FRAME_HEIGHT) << "\n";
	

	VideoWriter outputVideo;
	outputVideo.open ("../../../videos/output.mp4", VideoWriter::fourcc('M', 'J', 'P', 'G'), fps, Size(1280, 592));
	...
	while (true)
	{
		...
		*/
		...
		bool bSuccess = capture.read(RGB_img); // read a new frame from video 
		//Breaking the while loop at the end of the video
		if (bSuccess == false) break;

		logPolar(RGB_img, LogPolar_img, Point2f((float)RGB_img.cols / 2, (float)RGB_img.rows / 2),40,WARP_FILL_OUTLIERS);

		//Point2f center((float)img.cols / 2, (float)img.rows / 2);
		//double maxRadius = 0.7 * min(center.y, center.x);


		//warpPolar(img, img, img.size(), center, maxRadius, INTER_LINEAR + WARP_FILL_OUTLIERS + WARP_POLAR_LOG + WARP_INVERSE_MAP);

		outputVideo.write(LogPolar_img) ;


		/*
		 The imshow() function requires that a named window already exist(created by cvNamedWindow()).
		 */
		imshow("Second_Program_AVI_Video", LogPolar_img);
